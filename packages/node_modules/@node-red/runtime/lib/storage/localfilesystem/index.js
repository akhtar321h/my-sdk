...
var defaultFlow = require("./testFlow.json");
...
var localfilesystem = {
    init: async function(_settings, runtime) {
        ...
        if (!settings.userDir) {
            ...
            } else if (process.env.HOMEPATH && checkForConfigFile(fspath.join(process.env.HOMEPATH,".tIOTSDK"))) {
                settings.userDir = fspath.join(process.env.HOMEPATH,".tIOTSDK");
            } else {
                settings.userDir = fspath.join(process.env.HOME || process.env.USERPROFILE || process.env.HOMEPATH || process.env.NODE_RED_HOME,".tIOTSDK");
            }
        }
        ...
        var myFlowFile = fspath.join(settings.userDir,"testFlow.json");
        ...
        if (!settings.readOnly) {
            try {
                ...
                var defaultPackage = {
                    "name": "Trinity IOT-SDK-Project",
                    "description": "A Trinity IOT-SDK Project",
                    "version": "0.0.1",
                    "private": true
                };
                await util.writeFile(packageFile,JSON.stringify(defaultPackage,"",4));
                //Creating default flow file inside package
                return util.writeFile(myFlowFile,JSON.stringify(defaultFlow,"",4));
            }
        }
    },
